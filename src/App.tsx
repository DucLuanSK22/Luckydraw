...
export default function App() {
  ...
  const startSpin = useCallback(() => {
    if (isSpinning || availableParticipants.length === 0 || (currentPrize && currentPrize.remaining <= 0)) {
      if (availableParticipants.length === 0) showNotification('Không còn người tham gia hợp lệ cho giải này!', 'error');
    ...
    spinIntervalRef.current = window.setInterval(() => {
      ...
      setDisplayPerson(availableParticipants[randomIndex]);
    }, 40); // Faster spin
  ...
  const handlePasswordSubmit = () => {
    if (passwordInput.trim() === 'tap') {
      if (deleteWinnerTarget) {
        ...
        setTempPrizes(resetPrizes); // Update tempPrizes explicitly for Settings Modal UI
        setDisplayPerson(null);
            ...
            <div className="p-6 overflow-y-auto space-y-8">
              ...
              <div className="space-y-6">
                <h3 className="text-sm font-bold text-yellow-500 uppercase flex items-center gap-2">
                  <TypeIcon size={16} /> Tùy chỉnh hiển thị
                </h3>
              ...
              {/* Participant Lists */}
              <div className="space-y-4">
                <label className="text-sm font-medium text-slate-400">Danh sách người tham gia (CSV: Tên,GEN,SĐT)</label>
                <div className="space-y-4">
                  {tempPrizes.map((prize, idx) => (
                    <div key={prize.id} className="space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="text-xs font-bold text-yellow-500 uppercase">{prize.name}</span>
                        <span className="text-[10px] text-slate-500">{prize.list.length} người</span>
                      </div>
                      <textarea 
                        rows={3}
                        placeholder="Nguyễn Văn A,88888888,0901234567"
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
            ...
            >
              ...
              <input
                ...
                className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-center text-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all"
                autoFocus
              />
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-yellow-500 hover:bg-yellow-400 text-slate-900 rounded-xl font-bold transition-all"
                >
                  Xác nhận
                </button>
              </div>
